...
def run_knn_imputation(
    ...
    """
    Executa imputação via KNN a partir de um arquivo .npz contendo:
    ...
    Salva um novo .npz com as séries imputadas e métricas.
    """
    # ---- Carregar dados ----
    ...
    if original.ndim == 3:
        ...
        # Vamos considerar cada instante de tempo como um sample,
        # e concatenar N*C como features.
        ...
        # Caso (S, W, N, C): samples x janela x nós x canais (seu caso do SPIN)
        S, W, N, C = original.shape
        # Consideramos cada janela inteira (W,N,C) como um sample.
        # Flatten W*N*C em features.
    ...
    if output_path is None:
        # Ex.: spin_gatodomato_test_25pct.npz -> spin_gatodomato_test_25pct_knn.npz
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        description="Baseline de imputação KNN para os experimentos Gato do Mato usando arquivos .npz do SPIN."
    )
    parser.add_argument(
        "--input",
        type=str,
        required=True,
        help="Caminho para o arquivo .npz do experimento SPIN (ex.: spin_gatodomato_test_25pct.npz)",
    )
    parser.add_argument(
        "--neighbors",
        type=int,
    ...
    parser.add_argument(
        ...
        help=(
            ...
            "(ex.: spin_gatodomato_test_25pct_knn.npz)."
        ),
    )
